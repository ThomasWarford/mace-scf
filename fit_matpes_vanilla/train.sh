#!/bin/bash
#SBATCH --job-name=matpes_vanilla_debug
...
#SBATCH --output=fit_matpes_vanilla/logs/%x_%j.out
#SBATCH --error=fit_matpes_vanilla/logs/%x_%j.err

# Vanilla ScaleShiftMACE control for the LocalSplitCharges fits, same data and schedule.
#
#   sbatch fit_matpes_vanilla/train.sh                       # 2-node debug smoke test
#   sbatch --job-name=matpes_vanilla \
#          --qos=regular --time=12:00:00 \
#          fit_matpes_vanilla/train.sh                       # production
#
...
if [ "${SLURM_SUBMIT_DIR:-$ROOT}" != "$ROOT" ]; then
    ...
    echo "      under that directory, not $ROOT/fit_matpes_vanilla/logs/" >&2
fi

# Absolute, so nothing downstream depends on the working directory.
W="$ROOT/fit_matpes_vanilla"
