#!/bin/bash
#SBATCH --job-name=matpes_lsc_data_aug_debug
...
#SBATCH --output=fit_matpes_lsc_baseline_data_aug/logs/%x_%j.out
#SBATCH --error=fit_matpes_lsc_baseline_data_aug/logs/%x_%j.err

# Data-augmentation variant of fit_matpes_lsc_baseline: same everything, plus formal-charge noise (42% of MatPES frames have placeholder all-zero charges).
# Its own work dir, --name tag and job name, so it shares the W&B project with the baseline but never its checkpoints.
#
#   sbatch fit_matpes_lsc_baseline_data_aug/train.sh                       # 2-node debug smoke test
#   sbatch --job-name=matpes_lsc_data_aug \
#          --qos=regular --time=12:00:00 \
#          fit_matpes_lsc_baseline_data_aug/train.sh                       # production
#
...
if [ "${SLURM_SUBMIT_DIR:-$ROOT}" != "$ROOT" ]; then
    ...
    echo "      under that directory, not $ROOT/fit_matpes_lsc_baseline_data_aug/logs/" >&2
fi

# Absolute, so nothing downstream depends on the working directory.
W="$ROOT/fit_matpes_lsc_baseline_data_aug"
